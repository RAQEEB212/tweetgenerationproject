...
export const addEvent = createAsyncThunk('appCalendar/addEvent', async (event, { dispatch }) => {
  ...
  await dispatch(fetchEvents(['Drafts', 'Schuduled', 'Posts', 'Holiday', 'ETC']))
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: null,
    selectedCalendars: ['Drafts', 'Schuduled', 'Posts', 'Holiday', 'ETC']
  },
  reducers: {
    ...
    handleAllCalendars: (state, action) => {
      ...
      if (value === true) {
        state.selectedCalendars = ['Drafts', 'Schuduled', 'Posts', 'Holiday', 'ETC']
      } else {
